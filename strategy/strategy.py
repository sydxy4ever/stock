...
def init_result_db(rconn):
    cur = rconn.cursor()
    cur.execute("PRAGMA table_info(strategy)")
    cols_in_db = [row[1] for row in cur.fetchall()]
    if cols_in_db and "d1_close" not in cols_in_db:
        print("\n⚠️ Schema changed! Dropping old strategy table.")
        rconn.execute("DROP TABLE IF EXISTS strategy")
...
def save_to_result_db(rconn, df):
    ...
    rconn.executemany(f"INSERT INTO strategy ({','.join(cols)}) VALUES ({','.join(['?']*len(cols))}) ON CONFLICT(day0, stock_code) DO UPDATE SET {update_set}", rows)
    rconn.commit()
