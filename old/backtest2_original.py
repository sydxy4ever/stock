...
def load_stock_data():
    ...
    query = "SELECT code, date, close, to_r, to_avg_30, to_avg_5 FROM daily_kline ORDER BY code, date"
...
def main():
    ...
    print(f"共有 {len(start_dates)} 个有效起始日期")

    all_results = []
    for i, start_date in enumerate(start_dates):
        if i % 100 == 0:
            print(f"处理进度: {i}/{len(start_dates)}，当前起始日: {start_date}")
    ...
    if all_results:
        ...
        for day in range(1, 6):
            ...
            print(f"观察期第{day}天平均涨跌幅: {avg_pct:.4%}")
    else:
        print("没有符合条件的记录")
